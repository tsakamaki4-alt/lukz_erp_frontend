...
export default function MaterialsPage() {
  ...
  const currentClass = params.class as string; // 'raw_materials' or 'packaging'
  const searchPartNum = searchParams.get('search');

  // Requirement: Packaging filters only Packaging class. Raw Materials shows all.
  const isPackagingView = currentClass === 'packaging';
  const displayTitle = isPackagingView ? "Packaging" : "Raw Materials";
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let finalData = partsData;
      if (isPackagingView) {
        finalData = partsData.filter(p => p.product_class === 'Packaging');
      }

      const sortedData = [...finalData].sort((a, b) => (b.id || 0) - (a.id || 0));
      setParts(sortedData);
    } catch (error: any) {
  ...
  // Re-fetch when the URL segment changes
  useEffect(() => { 
    fetchData(); 
  ...
  /**
   * Logic: URL Search Synchronization
   * Automatically selects the part if the 'search' query param is present in URL
   */
  useEffect(() => {
    if (searchPartNum && parts.length > 0) {
      const partToSelect = parts.find(p => p.part_num === searchPartNum);
      if (partToSelect && selectedPart?.part_num !== searchPartNum) {
        setSelectedPart(partToSelect);
        setOriginalSnapshot(partToSelect);
        setActiveTab('General');
      }
    }
  }, [searchPartNum, parts, selectedPart?.part_num]);
  ...
  const handleSelectPart = (part: Part) => {
    // Logic: Updated to /parts/${currentClass}
    router.push(`/parts/${currentClass}?search=${encodeURIComponent(part.part_num)}`);
  ...
  const handleAddNew = () => {
    // Logic: Updated to /parts/${currentClass}
    router.push(`/parts/${currentClass}`);
    const newPart = { 
        ...EMPTY_PART, 
        product_class: isPackagingView ? 'Packaging' : '' 
    };
    setSelectedPart(newPart);
    setOriginalSnapshot(newPart);
    setActiveTab('General');
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Logic: Sync URL with /parts/${currentClass} after save
      if (isNew) {
         router.push(`/parts/${currentClass}?search=${encodeURIComponent(savedData.part_num)}`);
      }
    } catch (error: any) {
              ...
              <div className="flex flex-col h-full bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-3 border-b border-slate-100 flex flex-col gap-2 flex-shrink-0">
                  ...
                  >
                    ...
                    Create {isPackagingView ? 'Packaging' : 'Part'}
                  </button>
                  <div className="relative flex items-center">
                    <Search className="absolute left-3 text-slate-400" size={14} />
                    <input type="text" placeholder={`Query ${displayTitle}...`} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/10 transition-all" />
                  </div>
                </div>
              ...
              <div className="flex flex-col h-full bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden relative">
                {selectedPart ? (
                  <>
                    <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white flex-shrink-0">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h2 className="text-sm font-black text-slate-800 uppercase tracking-tight">{displayTitle} Master</h2>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
                    ...
                    <p className="text-xs font-bold uppercase tracking-[0.2em]">Select {displayTitle} to Modify</p>
                    <button 
                      onClick={handleAddNew}
